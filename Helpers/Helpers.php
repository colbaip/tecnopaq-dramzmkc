...
function headerEcommerce($data="")
{
   $view_header = "Views/Template/header_ecommerce.php";
   require_once($view_header);
}


function footerEcommerce($data="")
{
   $view_footer = "Views/Template/footer_ecommerce.php";
   require_once($view_footer);
}
...
function strClean($strCadena){
    ...
    function CurlConnectionGet(string $ruta, string $contentType=null, string $token){
        $content_type = $contentType != null ? $contentType : "application/x-wwww-form-urlencoded";
        if($token != null){
    ...
    function viewPage(int $idpagina){
        require_once("Libraries/Core/Mysql.php");
        $con = new Mysql();
        $sql = "SELECT * FROM pagina WHERE idpagina = $idpagina AND status != 0 ";
        $request = $con->select($sql);

        if( ($request['status'] == 1 AND isset($_SESSION['permisosMod']) AND $_SESSION['permisosMod']['u'] == true) OR
             $request['status'] != 2) {
              return true;
        }else{
              return false;
    ...
    function slugify(string $text): string {
        ...
        $text = trim($text);
        $text = mb_strtolower($text); // convierte a minúsculas
        $text = preg_replace('/[^a-z0-9\s-]/u', '', $text); // elimina símbolos no permitidos
        $text = preg_replace('/[\s]+/', '-', $text); // reemplaza espacios por guiones
        $text = preg_replace('/-+/', '-', $text); // evita guiones duplicados
        return $text;
    }
