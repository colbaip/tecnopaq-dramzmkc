...
require_once("Models/Traits/TTipoPago.php"); 
      ...
      class Pedidos extends Controllers{
            ...
            if($_SESSION['permisosMod']['r']){
                ...
                for ($i=0; $i < count($arrData); $i++){
                        ...
                        if($_SESSION['permisosMod']['r']){
                            ...
                            $btnView .= '<a title="Ver Detalle" href=" '.base_url().'/pedidos/orden/'.$arrData[$i]['idpedido'].'" target="_blanck" class="btn btn-info btn-sm"> 
                            <i class="far fa-eye"></i> </a>
                            <a title="Generar PDF"  href="'.base_url().'/factura/generarFactura/'.$arrData[$i]['idpedido'].'" target="_blanck" class="btn btn-warning btn-sm"><i class="fas fa-file-pdf"></i> </a> ';

                            // Para mostrar transaccion Paypal
                            // if($arrData[$i]['idtipopago'] ==1){
    
                            //     $btnView .=  '<a title="Ver Transaccion" href=" '.base_url().'/pedidos/transaccion/'.$arrData[$i]['idpedido'].'" target="_blanck" class="btn btn-info btn-sm"> 
                            //     <i class="fa fa-paypal" aria-hidden="true"></i></a>';
                            // }else{
                            //     $btnView .= '<button class="btn btn-secondary btn-sm" disable=""><i class="fa fa-paypal" aria-hidden="true"></i></button>';
                            // } 
                     ...
                     if($_SESSION['permisosMod']['u']){
                                 $btnEdit = '<button class="btn btn-primary btn-sm btnEditUsuario" onClick="fntEditInfo(this,'.$arrData[$i]['idpedido'].')"
                                             title="Editar Pedido"><i class="fas fa-pencil-alt"></i> </button>';
                     }
                     if($_SESSION['permisosMod']['d']){
                        
                                 $btnDelete = '<button class="btn btn-danger btn-sm btnDelUsuario" onClick="fntDelPedido('.$arrData[$i]['idpedido'].')"
        ...
        public function transaccion($idpedido)
        {
                ...
                $data['page_tag'] = "Detalles de la transaccion - Tienda Virtual";
                $data['page_title'] = "Detalles de la transacción";
                $data['page_name'] = "Detalles_transaccion";
                $data['arrPedido'] = $this->model->selectPedido($idpedido,$idpersona);
                //dep($data['arrPedido']);
                $this->views->getView($this,"orden", $data);
