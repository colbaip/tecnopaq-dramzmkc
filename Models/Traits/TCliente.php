...
trait TCliente {
    ...
    public function insertDetalleTemp(array $pedido)
    {
        ...
        $this->con = new Mysql();
        $sql = "SELECT * FROM detalle_tem WHERE
        ...
        if(empty($request))
        {
            foreach ($productos as $producto){
                $query_insert = "INSERT INTO detalle_tem(personaid,productoid,precio,cantidad,transaccionid)
                    VALUES (?,?,?,?,?)";
                $arrData = array(
                                $this->intIdUsuario,
                                $producto['idproducto'],
         ...
         $sqlDel = "DELETE FROM detalle_tem WHERE
                transaccionid = '{$this->intIdTransaccion}' AND
                personaid = $this->intIdUsuario";
    ...
    public function setContacto(string $nombre, string $email, string $mensaje, string $ip, string $dispositivo,
            string $useragent){
		$this->con = new Mysql();
